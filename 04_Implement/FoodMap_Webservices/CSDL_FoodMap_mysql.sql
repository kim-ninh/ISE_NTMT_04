...
BEGIN
	...
	DELETE FROM RANK WHERE RANK.ID_REST = ID_REST AND RANK.EMAIL_GUEST = EMAIL_GUEST;
	-- thêm mới vào
...
-- xóa owner
DELIMITER //
CREATE PROCEDURE SP_DELETE_OWNER(IN USERNAME VARCHAR(20))
BEGIN
...
BEGIN
	...
	IF EXISTS (SELECT * FROM DISCOUNT WHERE DISCOUNT.ID = IDDISCOUNT AND DISCOUNT.TIMEEND > NOW()) THEN
		-- thêm offer vào
		INSERT INTO OFFER (ID_DISCOUNT, GUEST_EMAIL, TOTAL, STATUS) VALUES(IDDISCOUNT, GUESTEMAIL, TOTAL, 0); 
		RETURN 1; -- true
	END IF;
...
BEGIN
	...
	IF EXISTS (SELECT * FROM DISCOUNT WHERE DISCOUNT.ID = IDDISCOUNT AND DISCOUNT.TIMEEND > NOW()) THEN
		-- cập nhật checkin
		UPDATE CHECKIN SET TOTAL_CHEKING = TOTAL_CHEKING + 1 WHERE CHECKIN.GUEST_EMAIL = GUESTEMAIL AND CHECKIN.ID_REST = ID_REST; 
	ELSE
		INSERT INTO CHECKIN (ID_REST, GUEST_EMAIL) VALUES (ID_REST, GUESTEMAIL);
	END IF;
...
BEGIN
	...
	IF EXISTS (SELECT * FROM DISCOUNT WHERE DISCOUNT.ID = IDDISCOUNT AND DISCOUNT.TIMEEND > NOW()) THEN
		-- cập nhật share
		UPDATE SHARE SET TOTAL_SHARE = TOTAL_SHARE + 1 WHERE SHARE.GUEST_SHARE = GUESTEMAIL AND SHARE.ID_REST = ID_REST; 
	ELSE
		INSERT INTO SHARE (ID_REST, GUEST_EMAIL) VALUES (ID_REST, GUESTEMAIL);
	END IF;
