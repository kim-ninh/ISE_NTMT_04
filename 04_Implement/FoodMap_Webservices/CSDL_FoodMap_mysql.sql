...
(
	...
	TIMESTART DATE NOT NULL, -- THỜI GIAN BẮT ĐẦU /// dd-MM-yyyy HH:mm:ss
	TIMEEND DATE NOT NULL  -- THỜI GIAN KẾT THỨC // dd-MM-yyyy HH:mm:ss
);
...
BEGIN
	DECLARE OWNER_USERNAME VARCHAR(20) DEFAULT 'NULL';
	...
	SELECT RESTAURANT.OWNER_USERNAME  INTO OWNER_USERNAME FROM RESTAURANT WHERE RESTAURANT.ID = ID_REST;
	DELETE FROM RESTAURANT WHERE RESTAURANT.ID = ID_REST;
	-- xóa người dùng
	DELETE FROM AUTHENTICATED WHERE AUTHENTICATED.USERNAME = OWNER_USERNAME;
	DELETE FROM OWNER WHERE OWNER.USERNAME = OWNER_USERNAME;
END //
DELIMITER ;
